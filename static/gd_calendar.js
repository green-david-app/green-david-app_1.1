...
window.GD_Calendar = (function(){
  ...
  async function mount(root){
    ...
    function draw(events){
      ...
      Array.from(grid.querySelectorAll(".cal-cell")).forEach(c=>{c.onclick=()=>{selected=c.getAttribute("data-date"); sel.textContent=selected; root.querySelector("#f_date").value=selected; renderList(events.filter(e=>e.start===selected));}});

...
window.GD_Calendar=(function(){
  ...
  function draw(root,ym,events){
    ...
    days.forEach(d=>{const k=iso(d), inM=(d.getMonth()+1)===ym.m; const c=document.createElement('div'); c.className='cal-cell'+(inM?'':' dim')+(k===today?' today':'');
      c.dataset.date=k; const de=document.createElement('div');de.className='cal-date';de.textContent=d.getDate();

...
class AIOperatorDrawer {
    ...
    addHeaderButton() {
        ...
        if (document.getElementById('ai-operator-header-btn')) return;

        // Vytvoř tlačítko
        const btn = document.createElement('button');
        btn.id = 'ai-operator-header-btn';
        btn.className = 'ai-operator-btn';
        btn.title = 'AI Operátor';
        btn.innerHTML = `
            ${this.icons.brain}
            <span class="badge" id="ai-operator-badge">
                <span class="badge-critical" id="ai-badge-critical" style="display:none">0</span>
                <span class="badge-warn" id="ai-badge-warn" style="display:none">0</span>
            </span>
            <span class="offline-indicator" id="ai-offline-indicator" style="display:none">⚡</span>
        `;
        btn.addEventListener('click', () => this.toggle());

        // Vlož před notifications
        const notifLink = headerActions.querySelector('.app-header-notifications');
        if (notifLink) {
            headerActions.insertBefore(btn, notifLink);
        } else {
    ...
    async logDecision(insightId, action, outcome) {
        ...
        try {
            ...
            console.log('Learning log failed (non-critical)');
        }
    }

...
class SmartPlanner {
    ...
    async loadData() {
        try {
            ...
            this.data.warehouseAlerts = Array.isArray(warehouse) ? warehouse : [];
    ...
    analyzeData(todayStr) {
        ...
        if (this.data.warehouseAlerts.length > 0) {
            analysis.warnings.push({
                type: 'lowStock',
                severity: 'medium',
...
plannerStyles.textContent = `
    ...
    .score-circle.good { background: rgba(159, 212, 161, 0.2); color: var(--mint, #9FD4A1); }

...
class AIJobsIntegration {
    ...
    injectAIPanel() {
        // Render into existing #ai-panel in the Overview tab
        const existingPanel = document.getElementById('ai-panel');
        
        if (!existingPanel) {
            console.warn('AI Jobs: #ai-panel not found in page');
            return;
        }

        // Add the class for styling and set content
        existingPanel.classList.add('ai-job-panel');
        existingPanel.innerHTML = this.renderAIPanelHTML();
        ...
        return `
            <div class="ai-panel-header" onclick="aiJobsIntegration.togglePanel()">
                <div class="ai-panel-title">
                    <span class="ai-icon" style="display:inline-flex;align-items:center;"><svg viewBox="0 0 24 24" fill="none" stroke="#4ade80" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="18" height="18"><circle cx="12" cy="12" r="3"/><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg></span>
    ...
    async fetchRisks() {
        try {
            ...
            if (!response.ok) {
                console.warn('AI risks endpoint not available');
                return [];
            }
            const data = await response.json();
            return data.risks || [];
        } catch (e) {
            console.warn('AI risks not available:', e);
            return [];
        }
    }
    ...
    async fetchCausalAnalysis() {
        try {
            ...
            if (!response.ok) {
                console.warn('AI causal endpoint not available');
                return null;
            }
            return await response.json();
        } catch (e) {
            console.warn('AI causal not available:', e);
            return null;
        }
    }
    ...
    async fetchProbabilityMap() {
        try {
            ...
            if (!response.ok) {
                console.warn('AI probability-map endpoint not available');
                return null;
            }
            return await response.json();
        } catch (e) {
            console.warn('AI probability-map not available:', e);
            return null;
        }
    }
    ...
    async fetchDecisionHistory() {
        try {
            ...
            if (!response.ok) {
                console.warn('AI decisions endpoint not available');
                return [];
            }
            const data = await response.json();
            return data.decisions || [];
        } catch (e) {
            console.warn('AI decisions not available:', e);
            return [];
        }
    }
    ...
    renderRisks() {
        ...
        if (risks.length === 0) {
            container.innerHTML = '<div class="no-risks">✓ Žádná významná rizika</div>';
            return;
        }
    ...
    renderRecommendations() {
        ...
        if (recommendations.length === 0) {
            container.innerHTML = '<div class="no-recommendations">Vše v pořádku 👍</div>';
            return;
        }
    ...
    renderHealthScore(score) {
        ...
        if (pathEl) {
            ...
            pathEl.setAttribute('class', 'health-ring-fill ' + this.getHealthClass(score));
        }
    }

...
window.GD_Timesheets=(function(){
  ...
  async function mount(root){
    root.innerHTML=`
      <div class="grid">
        <div class="card"><div class="card-h">Výkaz hodin zaměstnance</div><div class="card-c">
        ...
        <div class="card"><div class="card-h">Přidat záznam</div><div class="card-c">
    ...
    const employees=(await jfetch('/gd/api/employees')).employees||[]; const jobs=(await jfetch('/gd/api/jobs')).jobs||[];
    ...
    async function loadRows(){const emp=empSel.value; if(!emp){tbody.innerHTML=''; totalEl.textContent='0'; exp.classList.add('disabled'); exp.href='#'; return;}
      const f=root.querySelector('#rep_from').value||'', t=root.querySelector('#rep_to').value||'';
      const j=await jfetch(`/gd/api/reports/employee_hours?employee_id=${emp}&from=${f}&to=${t}`);
    ...
    root.querySelector('#rep_add').onsubmit=async ev=>{ev.preventDefault(); msg.style.display='none';
      ...
      try{await jfetch('/gd/api/timesheets',{method:'POST', body:JSON.stringify(d)}); msg.textContent='Zapsáno.'; msg.className='alert ok'; msg.style.display='block'; (ev.target).reset(); if(String(d.employee_id)===empSel.value){await loadRows();}}

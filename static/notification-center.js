...
(function() {
  ...
  function init() {
    ...
    if (!headerActions) {
      // Retry later if header not yet loaded
      setTimeout(init, 500);
      return;
    }
    ...
    container.innerHTML = `
      <button class="notif-bell" onclick="window.NotificationCenter.toggle()" title="Notifikace">
        ${bellIcon}
    ...
    const settingsLink = headerActions.querySelector('a[href*="settings"]');
    if (settingsLink) {
      headerActions.insertBefore(container, settingsLink);
    } else {

...
def api_nursery_overview():
    ...
    try:
        db = get_db()
        total = db.execute('SELECT COUNT(*) FROM nursery_plants WHERE status="active"').fetchone()[0]
        total_qty = db.execute('SELECT COALESCE(SUM(quantity),0) FROM nursery_plants WHERE status="active"').fetchone()[0]
        value = db.execute('SELECT COALESCE(SUM(quantity * COALESCE(selling_price,0)),0) FROM nursery_plants WHERE status="active"').fetchone()[0]
        return jsonify({'total_species': total, 'total_quantity': total_qty, 'total_value': value})

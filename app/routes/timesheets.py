...
def api_timesheets():
    ...
    if request.method == "GET":
        ...
        if 'performance_signal' in timesheet_cols:
            ...
            new_cols.append("t.performance_signal AS performance")  # Alias pro kompatibilitu
        ...
        if 'weather_snapshot' in timesheet_cols:
            new_cols.append("t.weather_snapshot AS weather")  # Alias pro kompatibilitu
        ...
        for r in rows:
            ...
            # Přidej aliasy pro kompatibilitu
            if 'performance_signal' in row_dict:
                row_dict['performance'] = row_dict['performance_signal']
            if 'weather_snapshot' in row_dict:
                row_dict['weather'] = row_dict['weather_snapshot']
    ...
    if request.method == "POST":
        try:
            ...
            # Podpora obou názvů: weather i weather_snapshot
            weather_data = data.get("weather") or data.get("weather_snapshot")
            weather_snapshot = json_lib.dumps(weather_data) if weather_data else None
            # Podpora obou názvů: performance i performance_signal
            performance_signal = data.get("performance") or data.get("performance_signal") or "normal"
    ...
    if request.method == "PATCH":
        try:
            ...
            for k in allowed:
                ...
                # Mapuj aliasy na skutečné názvy sloupců
                db_col = k
                if k == "performance":
                    db_col = "performance_signal"
                elif k == "weather":
                    db_col = "weather_snapshot"
                
                # Zkontroluj existenci sloupce
                if db_col not in timesheet_cols and k not in ["hours", "place", "activity", "performance", "weather"]:
                    continue
                ...
                elif k in ("material_used", "weather_snapshot", "weather", "ai_flags"):
                    v = json_lib.dumps(v) if v else None
                
                sets.append(f"{db_col}=?"); vals.append(v)

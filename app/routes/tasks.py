    ...
    if request.method == "POST":
        try:
            ...
            if created_by_in_cols:
                ...
                db.execute("""
                    INSERT INTO tasks(job_id, employee_id, title, description, status, due_date, priority, created_at, updated_at)
                    VALUES (?,?,?,?,?,?,?,datetime('now'),datetime('now'))
                """, (
                    int(data.get("job_id")) if data.get("job_id") else None,
                    int(data.get("employee_id")) if data.get("employee_id") else None,
...
def api_task_filters():
    ...
    # Zkontroluj existenci sloupce deadline (před všemi filtry)
    task_cols = [r[1] for r in db.execute("PRAGMA table_info(tasks)").fetchall()]
    ...
    if filter_type == "my_today":
        ...
        rows = db.execute(base_query + f"""
            INNER JOIN task_assignments ta ON ta.task_id = t.id
            WHERE ta.employee_id = ?
            AND DATE({deadline_col}) = DATE('now')
            AND t.status != 'completed'
            ORDER BY {deadline_col} ASC, t.id DESC
        """, (u["id"],)).fetchall()
        ...
        rows = db.execute(base_query + f"""
            INNER JOIN task_assignments ta ON ta.task_id = t.id
            WHERE ta.employee_id = ?
            AND DATE({deadline_col}) BETWEEN DATE('now') AND DATE('now', '+7 days')
            AND t.status != 'completed'
            ORDER BY {deadline_col} ASC
        """, (u["id"],)).fetchall()

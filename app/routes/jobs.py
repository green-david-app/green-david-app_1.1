...
def api_jobs():
    ...
    if request.method == "POST":
        try:
            ...
            job_type = data.get("job_type", "client")
            cols, vals = _job_insert_cols_and_vals(title, client, status, city, code, dt, note, owner_id=u["id"], party_id=party_id, job_type=job_type)
            sql = "INSERT INTO jobs(" + ",".join(cols) + ") VALUES (" + ",".join(["?"]*len(vals)) + ")"
            cur = db.execute(sql, vals)
            job_id = cur.lastrowid
            db.commit()
            
            # Přepočítej statistiky klienta pokud má party_id
            if party_id:
                try:
                    from app.routes.parties import recalculate_party_stats
                    recalculate_party_stats(party_id)
                except:
                    pass
            
            audit_event(u.get("id"), "create", "job", job_id, after={"title": title, "client": client, "status": status, "city": city, "code": code, "date": dt, "party_id": party_id})
    ...
    if request.method == "PATCH":
        try:
            ...
            for f in ("client","status","city","code","date","note","created_date","start_date","progress","deadline","address","location","estimated_value","budget","party_id","job_type"):
                ...
                if f in data:
                    if f in ("date", "created_date", "start_date", "deadline"):
                        ...
                        v = int(data[f]) if data[f] is not None else None
                    else:
            ...
            # Pokud se změnil party_id, přepočítej statistiky
            old_party_id = None
            new_party_id = data.get("party_id")
            if "party_id" in data:
                try:
                    old_job = db.execute("SELECT party_id FROM jobs WHERE id=?", (jid,)).fetchone()
                    if old_job:
                        old_party_id = old_job["party_id"]
                except:
                    pass
...
def api_jobs_metrics_overview():
    ...
    # 2. Assignments pro všechny zakázky (check both tables)
    all_assignments = {}
    try:
        ...
        print(f"[OVERVIEW] job_assignments query failed: {e}")
    
    # Also check job_team_assignments (detailed table)
    try:
        jta_rows = db.execute("""
            SELECT jta.job_id, jta.employee_id, e.name, e.role
            FROM job_team_assignments jta
            JOIN employees e ON e.id = jta.employee_id
            WHERE jta.is_active = 1
        """).fetchall()
        for a in jta_rows:
            jid = a['job_id']
            if jid not in all_assignments:
                all_assignments[jid] = []
            # Avoid duplicates
            existing_ids = [x['employee_id'] for x in all_assignments[jid]]
            if a['employee_id'] not in existing_ids:
                all_assignments[jid].append({
                    'employee_id': a['employee_id'],
                    'name': a['name'],
                    'role': a['role']
        ...
        print(f"[OVERVIEW] job_team_assignments query failed: {e}")
...
def api_job_hub(job_id):
    ...
    try:
        ...
        if has_labor_cost:
            ts_rows = db.execute("""
                SELECT e.id as emp_id, e.name, e.hourly_rate,
                       SUM(t.hours) as hours,
                       COUNT(*) as entries,
                       SUM(COALESCE(t.labor_cost, 0)) as total_labor_cost
                FROM timesheets t
        ...
        for ts in ts_rows:
            ts = dict(ts)
            emp_hours = ts.get('hours') or 0
            emp_rate = ts.get('hourly_rate') or 250.0

            if has_labor_cost:
                emp_cost = ts.get('total_labor_cost') or 0
                if emp_cost == 0 and emp_hours > 0:
                    emp_cost = round(emp_hours * emp_rate, 0)
            else:
                emp_cost = round(emp_hours * emp_rate, 0)
            ...
            timesheets_summary['by_employee'].append({
                'employee_id': ts.get('emp_id'),
                'name': ts.get('name'),
                'hourly_rate': round(emp_rate, 0),
                'hours': round(emp_hours, 1),
                'cost': round(emp_cost, 0),
                'entries': ts.get('entries', 0)
            })

        timesheets_summary['total_hours'] = round(total_hours, 1)
        timesheets_summary['total_cost'] = round(total_cost, 0)
        timesheets_summary['entries'] = sum(t.get('entries', 0) for t in ts_rows)
    except Exception as e:
        print(f"[WARN] Timesheets summary error: {e}")
...
def api_tasks():
    ...
    if request.method == "GET":
        ...
        if task_id:
            ...
            # Zkontroluj existenci sloupců
            task_cols = [r[1] for r in db.execute("PRAGMA table_info(tasks)").fetchall()]
            priority_col = "COALESCE(t.priority, 'medium') as priority" if "priority" in task_cols else "'medium' as priority"
            deadline_col = "t.deadline" if "deadline" in task_cols else "t.due_date as deadline"
            
            row = db.execute(f"""SELECT t.id, t.job_id, t.employee_id, t.title, t.description, t.status, t.due_date, {deadline_col}, {priority_col},
        ...
        q = f"""SELECT t.id, t.job_id, t.employee_id, t.title, t.description, t.status, t.due_date, {deadline_col}, {priority_col},
                      e.name AS employee_name
               FROM tasks t
               LEFT JOIN employees e ON e.id=t.employee_id"""
        conds=[]; params=[]
        ...
        if employee_id:
            ...
            q = f"""SELECT DISTINCT t.id, t.job_id, t.employee_id, t.title, t.description, t.status, t.due_date, {deadline_col}, {priority_col},
                          e.name AS employee_name
                   FROM tasks t
                   LEFT JOIN employees e ON e.id=t.employee_id
    ...
    if request.method == "POST":
        try:
            ...
            # Zkontroluj existenci sloupce created_by před INSERT
            task_cols = [r[1] for r in db.execute("PRAGMA table_info(tasks)").fetchall()]
            created_by_in_cols = "created_by" in task_cols
            
            # Vytvoř úkol
            if created_by_in_cols:
                db.execute("""
...
# Helper functions for timesheets
def calculate_labor_cost(employee_id, job_id, duration_minutes, db):
    """Vypočítá náklady na práci na základě hodinové sazby zaměstnance"""
    try:
        cols = [r[1] for r in db.execute("PRAGMA table_info(employees)").fetchall()]
        if 'hourly_rate' in cols:
            emp = db.execute("SELECT hourly_rate FROM employees WHERE id=?", (employee_id,)).fetchone()
            rate = float(emp['hourly_rate']) if emp and emp['hourly_rate'] else 250.0
        else:
            rate = 250.0  # default 250 CZK/h
        return round(rate * (duration_minutes / 60.0), 2)
    except Exception:
        return 0.0

def detect_anomalies(data, db):
    """Detekuje anomálie ve výkazu (zjednodušená verze)"""
    flags = []
    duration = data.get('duration_minutes', 0) or 0
    if duration > 720:  # > 12 hodin
        flags.append({'type': 'long_shift', 'message': 'Směna delší než 12 hodin'})
    if duration < 15 and duration > 0:
        flags.append({'type': 'very_short', 'message': 'Velmi krátký záznam'})
    if data.get('delay_reason'):
        flags.append({'type': 'delay', 'message': f"Důvod zpoždění: {data.get('delay_reason')}"})
    return flags
...
# Weather API moved to api.py - removed duplicate
    ...
    try:
        ...
        try:
            ts_hours = db.execute("""
                ...
                GROUP BY employee_id
            """, (job_id,)).fetchall()
            ts_map = {r['employee_id']: {'hours': round(r['actual_hours'], 1), 'entries': r['entry_count']} for r in ts_hours}
            
            for t in team:
                emp_ts = ts_map.get(t.get('employee_id'), {})
                t['hours_actual'] = emp_ts.get('hours', t.get('hours_actual', 0))
                t['timesheet_entries'] = emp_ts.get('entries', 0)
        except Exception as e:
            print(f"[TEAM] Error enriching team hours: {e}")
...
def manage_job_location(job_id):
    ...
    try:
        if request.method == 'GET':
            ...
            location = dict(location_row) if location_row else {}
...
def manage_milestones(job_id):
    ...
    try:
        if request.method == 'GET':
            milestones_rows = db.execute("SELECT * FROM job_milestones WHERE job_id = ? ORDER BY order_num", (job_id,)).fetchall()

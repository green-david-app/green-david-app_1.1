...
def _job_select_all():
    ...
    jobtype_col = ", job_type" if "job_type" in info else ""
    if "title" in info:
        return f"SELECT title, {base_cols}{date_cols}{deadline_col}{address_col}{progress_col}{time_col}{budget_col}{cost_col}{party_col}{jobtype_col} FROM jobs"
    if "name" in info:
        return f"SELECT name AS title, {base_cols}{date_cols}{deadline_col}{address_col}{progress_col}{time_col}{budget_col}{cost_col}{party_col}{jobtype_col} FROM jobs"
    return f"SELECT '' AS title, {base_cols}{date_cols}{deadline_col}{address_col}{progress_col}{time_col}{budget_col}{cost_col}{party_col}{jobtype_col} FROM jobs"


def _job_insert_cols_and_vals(title, client, status, city, code, dt, note, owner_id=None, party_id=None, job_type=None):
    info = _jobs_info()
    cols = []
    vals = []
    ...
    # job_type pokud existuje
    if "job_type" in info and job_type is not None:
        cols.append("job_type"); vals.append(job_type)

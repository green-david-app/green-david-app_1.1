...
# ---------- EXTRA API: Calendar aggregation ----------
@app.get("/api/calendar")
def api_calendar():
    db = get_db()
    qdate = (request.args.get("date") or "").strip()
    out = {"date": qdate, "jobs": [], "tasks": [], "notes": [], "work_logs": []}

    def rows(sql, p=()): 
        return [dict(r) for r in db.execute(sql, p).fetchall()]

    if qdate:
        d = _normalize_date(qdate)
        out["date"] = d
        out["jobs"] = rows("SELECT * FROM jobs WHERE date=? ORDER BY id DESC", (d,))
        out["tasks"] = rows("SELECT * FROM tasks WHERE due_date=? ORDER BY id DESC", (d,))
        out["notes"] = rows("SELECT * FROM notes WHERE note_date=? ORDER BY id DESC", (d,))
        out["work_logs"] = rows("SELECT * FROM work_logs WHERE work_date=? ORDER BY id DESC", (d,))
        return jsonify({"ok": True, "data": out})

    rows_map = {}
    for tbl, col in (("jobs","date"),("tasks","due_date"),("notes","note_date"),("work_logs","work_date")):
        sql = f"SELECT {col} AS d, COUNT(*) AS c FROM {tbl} WHERE {col} IS NOT NULL AND {col}<>'' GROUP BY {col}"
        for r in db.execute(sql).fetchall():
            d = r["d"]
            rows_map.setdefault(d, {"date": d, "jobs":0,"tasks":0,"notes":0,"work_logs":0})
            key = 'work_logs' if tbl=='work_logs' else tbl
            rows_map[d][key] = r["c"]
    summary = sorted(rows_map.values(), key=lambda x: x["date"], reverse=True)[:60]
...
def api_worklogs():
    db = get_db()
    if request.method == "GET":
        items = [dict(r) for r in db.execute("SELECT * FROM work_logs ORDER BY work_date DESC, id DESC").fetchall()]
        return jsonify({"ok": True, "work_logs": items})
    data = request.get_json(silent=True) or {}
    employee = (data.get("employee_name") or "").strip()
    try:
        hours = float(data.get("hours") or 0)
    except Exception:
        hours = 0
    work_date = _normalize_date(str(data.get("work_date") or "").strip())
    job_title = (data.get("job_title") or "").strip()
    job_id = int(data.get("job_id") or 0) or None
    note = (data.get("note") or None)
    if not employee or hours <= 0 or not work_date:
        return jsonify({"ok": False, "error": "missing_fields"}), 400
    db.execute(
        "INSERT INTO work_logs(employee_name, job_id, job_title, hours, work_date, note, user_id) VALUES (?,?,?,?,?,?,1)",
        (employee, job_id, job_title, hours, work_date, note)
    )
    db.commit()
    return jsonify({"ok": True})

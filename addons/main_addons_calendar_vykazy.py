...
# === Schema: work_logs (safe create-if-not-exists) ===
...
def api_calendar():
    ...
    if qdate:
        ...
        return jsonify({
            ...
            "data": {
                "date": d,
                "jobs": rows("SELECT * FROM jobs WHERE date=? ORDER BY id DESC", (d,)),
                "work_logs": rows("SELECT * FROM work_logs WHERE work_date=? ORDER BY id DESC", (d,)),
            }
        })

    # summary counts per day
    summary = {}
    for tbl, col in (("jobs","date"),("work_logs","work_date")):
        sql = f"SELECT {col} AS d, COUNT(*) AS c FROM {tbl} WHERE {col} IS NOT NULL AND {col}<>'' GROUP BY {col}"
        for r in db.execute(sql):
            d = r["d"]
            if not d: 
                continue
            summary.setdefault(d, {"date": d, "jobs":0,"work_logs":0})
            key = "work_logs" if tbl=="work_logs" else tbl
            summary[d][key] = r["c"]
    ordered = sorted(summary.values(), key=lambda x: x["date"])
    return jsonify({"ok": True, "summary": ordered})
...
def api_worklogs():
    db = get_db()
    if request.method == "GET":
        items = [dict(r) for r in db.execute("SELECT * FROM work_logs ORDER BY work_date DESC, id DESC")]

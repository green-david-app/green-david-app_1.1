...
def ensure_gd_db():
    ...
    """)
    # seed employees minimally if empty
...
# ---------------- API: Employees ----------------
@GD_BP.route("/api/employees", methods=["GET","POST"])
...
@GD_BP.route("/api/employees/<int:eid>", methods=["PATCH","DELETE"])
...
# ---------------- API: Jobs ----------------
@GD_BP.route("/api/jobs", methods=["GET","POST"])
...
@GD_BP.route("/api/jobs/<int:jid>", methods=["PATCH","DELETE"])
...
# ---------------- API: Tasks ----------------
@GD_BP.route("/api/tasks", methods=["GET","POST"])
...
@GD_BP.route("/api/tasks/<int:tid>", methods=["PATCH","DELETE"])
...
@GD_BP.route("/api/timesheets", methods=["GET","POST"])
def timesheets_list_create():
    db=get_db()
    if request.method=="GET":
        rows=[dict(r) for r in db.execute("""
            SELECT t.*, j.title as job_title, j.code as job_code, e.name as employee_name
            FROM timesheets t JOIN jobs j ON j.id=t.job_id JOIN employees e ON e.id=t.employee_id
            ORDER BY date DESC, t.id DESC
        """)]
        return jsonify(ok=True, timesheets=rows)
    d=request.get_json(force=True, silent=True) or {}
    required = (d.get("employee_id"), d.get("job_id"), d.get("date"), d.get("hours") is not None)
    if not all(required):
...
@GD_BP.route("/export/employee_hours.csv")
...
# ---------------- API: Calendar ----------------
@GD_BP.route("/api/calendar", methods=["GET","POST","PATCH","DELETE"])
def api_calendar():
    db=get_db()
    if request.method=="GET":
    ...
    if request.method=="POST":
        ...
        if not title or not start: return jsonify(ok=False, error="missing_title_or_date"), 400

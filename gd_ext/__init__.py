# -*- coding: utf-8 -*-
# green-david gd_ext blueprint (fix3) - conservative syntax, no nested escapes
...
def ensure_gd_db():
    ...
    schema_sql = (
        ...
        " id INTEGER PRIMARY KEY AUTOINCREMENT,"
        " title TEXT NOT NULL, client TEXT, status TEXT, city TEXT, code TEXT, date TEXT, note TEXT"
        ");"
        "CREATE TABLE IF NOT EXISTS tasks ("
        " id INTEGER PRIMARY KEY AUTOINCREMENT,"
        " title TEXT NOT NULL, description TEXT, status TEXT, due_date TEXT, employee_id INTEGER, job_id INTEGER"
        ");"
        "CREATE TABLE IF NOT EXISTS timesheets ("
        " id INTEGER PRIMARY KEY AUTOINCREMENT,"
        " employee_id INTEGER NOT NULL, job_id INTEGER NOT NULL, date TEXT NOT NULL,"
        " hours REAL NOT NULL, place TEXT, activity TEXT"
        ");"
        "CREATE TABLE IF NOT EXISTS calendar_events ("
        " id INTEGER PRIMARY KEY AUTOINCREMENT,"
        " title TEXT NOT NULL, start TEXT NOT NULL, "end" TEXT,"
        " all_day INTEGER NOT NULL DEFAULT 1, type TEXT NOT NULL DEFAULT 'note',"
    ...
    if not db.execute("SELECT 1 FROM employees LIMIT 1").fetchone():
        db.executemany(
            ...
            [
                ("Pepa","pepa@greendavid.local",""),
                ("Misa","misa@greendavid.local",""),
                ("Kaja","kaja@greendavid.local",""),
            ],
        )
    db.commit()
    db.close()
...
def api_jobs():
    ...
    if request.method == "GET":
        rows = db.execute(
            "SELECT * FROM jobs ORDER BY COALESCE(date,'9999-12-31') DESC, id DESC"
        ).fetchall()
...
def api_tasks():
    ...
    if request.method == "GET":
        rows = db.execute(
            "SELECT * FROM tasks ORDER BY COALESCE(due_date,'9999-12-31') DESC, id DESC"
        ).fetchall()
    ...
    if not title or not due_date:
        return jsonify(ok=False, error="missing_fields"), 400
    cur = db.execute(
        "INSERT INTO tasks(title,description,status,due_date,employee_id,job_id) VALUES (?,?,?,?,?,?)",
...
def api_timesheets():
    ...
    if request.method == "GET":
        ...
        return jsonify(ok=True, timesheets=[dict(r) for r in rows])
    d = request.get_json(silent=True) or {}
    if not (d.get("employee_id") and d.get("job_id") and d.get("date") and (d.get("hours") is not None)):
        return jsonify(ok=False, error="missing_fields"), 400
    db.execute(
        "INSERT INTO timesheets(employee_id,job_id,date,hours,place,activity) VALUES (?,?,?,?,?,?)",
...
def api_report_employee_hours():
    ...
    rows = db.execute(
        ...
        "AND (? IS NULL OR date(t.date) <= date(?)) "
        "ORDER BY t.date ASC",
        (emp, dfrom, dfrom, dto, dto),
    ).fetchall()
    total = sum((r["hours"] or 0) for r in rows)
    return jsonify(ok=True, rows=[dict(r) for r in rows], total_hours=total)
...
def api_calendar():
    ...
    if request.method == "GET":
        ...
        if not q_from or not q_to:
            now = datetime.now()
            q_from = f"{now.year}-{now.month:02d}-01"
            if now.month == 12:
                y2, m2 = (now.year + 1, 1)
            else:
    ...
    if request.method == "POST":
        ...
        db.execute(
            "INSERT INTO calendar_events(title,start,"end",all_day,type,ref_id,notes) VALUES (?,?,?,?,?,?,?)",
    ...
    if request.method == "PATCH":
        ...
        if sets:
            vals.append(eid)
            sql = "UPDATE calendar_events SET " + ", ".join(sets) + " WHERE id=?"
            db.execute(sql, vals)
            db.commit()
        return jsonify(ok=True)

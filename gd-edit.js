...
(function(){
  ...
  // LIST: přidej "Upravit" vedle "Otevřít detail"
  function injectOnList(){
    qsa('button, a').forEach(el=>{
      ...
      if(t==='otevřít detail' || t==='otevrit detail'){
        ...
        jid = (jid.match(/\d+/)||[])[0]; if(!jid) return;
  ...
  // DETAIL: přidej pevné plovoucí tlačítko vpravo nahoře (nezávislé na struktuře)
  function injectFloatingOnDetail(){
    const id = getParam('id'); if(!id) return;
    if(document.getElementById('gd-float-edit')) return;
    const btn=document.createElement('button');
    btn.id='gd-float-edit'; btn.type='button'; btn.textContent='Upravit';
    btn.className='btn';
    btn.style.position='fixed'; btn.style.top='100px'; btn.style.right='24px'; btn.style.zIndex='9999';
    btn.style.padding='10px 14px'; btn.style.borderRadius='10px';
    btn.addEventListener('click', ()=>{ location.href='/' + '#edit-'+id; });
    document.body.appendChild(btn);
  }

  function hashTrigger(){ const m=(location.hash||'').match(/^#edit-(\d+)$/); if(m) setTimeout(()=>loadJobIntoForm(m[1]), 300); }
  ...
  function init(){ injectOnList(); injectFloatingOnDetail(); hashTrigger(); }

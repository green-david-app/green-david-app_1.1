...
# ----------------- Employees & Temps APIs -----------------
def _ensure_staff_table():
    db = get_db()
    db.execute("CREATE TABLE IF NOT EXISTS employees (id INTEGER PRIMARY KEY, name TEXT NOT NULL, role TEXT, active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL DEFAULT (datetime('now')))")
    db.commit()

def _ensure_temps_table():
    db = get_db()
    db.execute("CREATE TABLE IF NOT EXISTS temps (id INTEGER PRIMARY KEY, name TEXT NOT NULL, role TEXT, active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL DEFAULT (datetime('now')))")
    db.commit()

@app.get('/gd/api/staff')
def api_list_staff():
    require_auth()
    _ensure_staff_table()
    db = get_db()
    rows = [dict(r) for r in db.execute("SELECT id,name,role FROM employees WHERE IFNULL(active,1)=1 ORDER BY id DESC")]
    return jsonify(dict(ok=True, rows=rows))

@app.post('/gd/api/staff')
def api_add_staff():
    require_auth()
    _ensure_staff_table()
    ...
    db = get_db()
    cur = db.execute("INSERT INTO employees(name,role,active) VALUES(?,?,1)", (name, role))
    db.commit()
    return jsonify(dict(ok=True, id=cur.lastrowid))

@app.delete('/gd/api/staff/<int:emp_id>')
def api_del_staff(emp_id):
    require_auth()
    _ensure_staff_table()
    db = get_db()
...
@app.get('/gd/api/temps')
def api_list_temps():
    require_auth()
    _ensure_temps_table()
    db = get_db()
    rows = [dict(r) for r in db.execute("SELECT id,name,role FROM temps WHERE IFNULL(active,1)=1 ORDER BY id DESC")]
    return jsonify(dict(ok=True, rows=rows))

@app.post('/gd/api/temps')
def api_add_temp():
    require_auth()
    _ensure_temps_table()
    data = request.get_json(silent=True) or {}
    ...
    if not name:
        abort(400, description='name required')
    db = get_db()
    cur = db.execute("INSERT INTO temps(name,role,active) VALUES(?,?,1)", (name, role or 'brigádník'))
    db.commit()
    return jsonify(dict(ok=True, id=cur.lastrowid))

@app.delete('/gd/api/temps/<int:temp_id>')
def api_del_temp(temp_id):
    require_auth()
    _ensure_temps_table()
    db = get_db()
    db.execute("UPDATE temps SET active=0 WHERE id=?", (temp_id,))
    db.commit()
    return jsonify(dict(ok=True))

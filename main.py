...
def api_calendar_summary():
    """Return counts of jobs and timesheets per date in range [from, to]."""
    date_from = request.args.get("from")
    date_to = request.args.get("to")
    today = _dt.utcnow().date()
    start = _parse_date(date_from) or (today.replace(day=1))
    end = _parse_date(date_to) or (start.replace(day=28) + _td(days=10)).replace(day=1) - _td(days=1)

    conn = get_db()
    cur = conn.cursor()

    # Jobs per date
    cur.execute("SELECT date, COUNT(*) FROM jobs WHERE date BETWEEN ? AND ? GROUP BY date", (start.isoformat(), end.isoformat()))
    job_counts = {row[0]: row[1] for row in cur.fetchall()}

    # Timesheets per date
    cur.execute("SELECT date, COUNT(*) FROM timesheets WHERE date BETWEEN ? AND ? GROUP BY date", (start.isoformat(), end.isoformat()))
    ts_counts = {row[0]: row[1] for row in cur.fetchall()}
    ...
    while d <= end:
        ds = d.isoformat()
        data.append({"date": ds, "jobs": int(job_counts.get(ds, 0)), "work_logs": int(ts_counts.get(ds, 0))})
        d += _td(days=1)
...
def api_calendar_day():
    """Return jobs + timesheets for a specific date."""
    ...
    cur.execute("SELECT id, title, name, client, city, code, date, status FROM jobs WHERE date = ? ORDER BY LOWER(title)", (date,))
    jobs = []
    for row in cur.fetchall():
        jobs.append({
            "id": row[0], "title": row[1], "name": row[2], "client": row[3], "city": row[4],
            "code": row[5], "date": row[6], "status": row[7]
        })

    cur.execute("SELECT id, employee_name, job_title, date, hours, place, activity FROM timesheets WHERE date = ? ORDER BY employee_name", (date,))
    work_logs = [{
        "id": r[0], "employee_name": r[1], "job_title": r[2], "date": r[3], "hours": r[4], "place": r[5], "activity": r[6]
    } for r in cur.fetchall()]

    return jsonify({"ok": True, "data": {"date": date, "jobs": jobs, "work_logs": work_logs}})
...
def page_calendar():
    ...
    if err: return err
    return render_template("calendar.html", current_user=u)

@app.route("/vykazy")
def page_vykazy():
    u, err = require_role(write=True)  # only logged-in (write) users can add
    if err: return err
    return render_template("worklogs.html", current_user=u)
